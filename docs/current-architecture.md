...
|---|---|---|
| backend | NestJS + TypeScript + Prisma + PostgreSQL | Nest ^11.1.29, Prisma ^5.7.1, TS ^5.3.3 |
| frontend | React + MUI, Vite | React ^18.2.0, Vite ^8.2.1 (migrated off Create React App 2026-08-11, see `docs/roadmap.md` item 18) |
| public-website | React + MUI, Vite | React ^18.2.0, Vite ^8.2.1 (migrated off Create React App 2026-08-11, see `docs/roadmap.md` item 19) |
...
*(Updated 2026-08-11 — Phase 5.2 and Phase 6.5 split the largest page files into `hooks/` + presentational components; Phase 6.6 and 6.10 added test coverage and route-based code splitting; Phase 6.18 migrated the build tool off Create React App onto Vite. This section previously described the pre-split, zero-test, CRA-built state; it's now current.)*

- **Build tool: Vite (`vite.config.js`), not Create React App** (migrated 2026-08-11, see `docs/roadmap.md` item 18 — `public-website` migrated too, item 19, same day). Entry point is root-level `index.html` → `src/index.jsx` (not `public/index.html`; `frontend/public/` no longer exists — it held only `index.html` and `manifest.json`, both now generated/handled by Vite/`vite-plugin-pwa`). Env vars are `VITE_*`-prefixed (`VITE_API_URL`, was `REACT_APP_API_URL`), read via `import.meta.env.VITE_API_URL`, not `process.env`. Service worker registration is auto-injected by `vite-plugin-pwa` (`registerType: 'autoUpdate'`) instead of the old manual `src/serviceWorkerRegistration.js`. Tests run on Vitest (`vitest.config.js`), not Jest — `test`/`describe`/`it`/`expect`/`vi` are globals (`test.globals: true`), matching the old Jest ergonomics closely on purpose.

- Routing (`frontend/src/App.jsx`, `BrowserRouter`): `/partner/login`, `/partner/dashboard` (guarded by `ProtectedPartnerRoute`); everything else (`/`, `/bookings`, `/bookings/new`, `/bookings/:id`, `/stays`, `/customers`, `/equipment`, `/boat-prep`, `/schedule`, `/schedule/trip/:date/:type/:boatId?/:session?`, `/settings`, `/breaches`, `/bill`, `/partners`, `/partner-invoices`, `/bills`, `/financial`) wrapped in `ProtectedRoute`. **Phase 6.10:** every page is now `React.lazy()`-loaded (its own chunk, fetched on first visit) rather than statically imported — dropped the main bundle from 436KB to 176KB gzipped. Two `<Suspense>` boundaries share the same `CircularProgress` fallback `ProtectedPartnerRoute` already used.
...
- Structure: 17 page files (`pages/`, plus `pages/partner/`) — the largest untouched ones are `Customers.jsx` (950 lines), `TripDetails.jsx` (785), `Breaches.jsx` (749), `Stays.jsx` (711), `Partners.jsx` (664); `Settings.jsx`/`BoatPrep.jsx`/`Financial.jsx`/`Equipment.jsx`/`Schedule.jsx`/`Bill.jsx` — the six that were 1,100+ lines — were each split (Phase 5.2, Phase 6.5) into a `hooks/useXData.js` (all state/effects/handlers) plus presentational components/tabs/views under `components/X/`, leaving the `pages/X.jsx` file itself a thin router (tens of lines, not thousands). 43 component files under `components/` (`Auth`, `Bill`, `BoatPrep`, `Booking`, `Common`, `Customer`, `Dashboard`, `Equipment`, `Financial`, `Partner`, `Schedule`, `Settings`), 15 service files (`api/`, `apiService`, `bookingRepricingService`, `breachService`, `dataService`, `financialService`, `pricingService`, `sharedStorage`, `stayCostsService`, `stayService`, `syncService`, `tankService`), 9 util files including `authContext.js`, `scheduleConstants.js`. `hooks/` now exists (`useBillData.js`, `useBoatPrepData.jsx`, `useEquipmentData.js`, `useFinancialData.jsx`, `useScheduleData.js`) — state for the split pages lives there; unsplit pages still manage state ad hoc via local component state, same as before. Still no `context/` or `store/` folder beyond `authContext.jsx`/`languageContext.jsx`/`partnerAuthContext.jsx`/`tenantContext.js`. (Files with real JSX in them are `.jsx`, not `.js` — matters now under Vite's esbuild, which needs the extension to parse JSX by default; five files were renamed for this in the Vite migration, see item 18.)
- **Test coverage (Phase 6.6, targeted — not comprehensive):** 11 test files, 80 tests total. `*.smoke.test.jsx` for the split pages (`Customers`, `Financial`, `Bill`, `Schedule`, `Settings`, `Equipment`, `BoatPrep`) plus `authContext.test.js`, `ProtectedRoute.test.jsx`, `AdminLogin.test.jsx`, and `pricingService.test.js` (30 tests covering the tiered dive-pricing calculation logic). Ordinary CRUD pages not touched by a Phase 5.2/6.5 split or Phase 6.6's targeted pass (e.g. `Bookings.jsx`, `Stays.jsx`, `Partners.jsx`) remain untested.

## 6. public-website (customer-facing)

- **Build tool: Vite, not Create React App** (migrated 2026-08-11, see `docs/roadmap.md` item 19). Unlike `frontend`, no `vite-plugin-pwa` — `public-website/public/service-worker.js` is a real, hand-authored service worker (network-first for HTML/JS/CSS so deploys reach users immediately, cache-first for static assets, push-notification handling) rather than CRA's auto-generated workbox output, so it's left as a plain static file in `public/` (which Vite copies through to the build output untouched, same as CRA did) instead of being replaced by plugin-generated output. `public/manifest.json`, `logo.svg`, `pwa-icons/`, and `.htaccess` (Apache config for the OVH FTP-hosted production deploy) are all still there, unchanged. Still no test suite (a pre-existing gap noted below in §4.6's sibling note), so this migration had no Jest→Vitest step to do. `deploy/deploy_landing_to_ovh.py` (the actual OVH FTP production deploy path) and the two ZBOX Docker deploy scripts were updated for the `VITE_API_URL` rename.
...
- No `robots.txt`, no `sitemap.xml` in `public-website/public` (`frontend/public` no longer exists as of the Vite migration, item 18 — not that it ever had either file).
...
- **(2026-08-10)** Two more genuine forward migrations: `20260810180000_equipment_details_json` (adds `equipment.details` JSON column, Phase 6.14) and `20260810210000_schedule_slot_guides` (adds `bookings.moleSlotTime`/`session` plus the new `schedule_slot_guides` table, Phase 6.15).
- **(2026-08-10) Duplicate migration history cleaned up (Phase 6, item 12):** the baselining commit above had also swept in two pre-existing, never-committed local migration files (`20251227114043_add_partner_invoices`, `20251228212110_add_customer_bills`) alongside the new baseline. Content diff confirmed `0_baseline` was already a strict superset of both (created every enum/table they did, plus later additions, with the corrected schema) — a from-scratch replay (shadow database, or a genuinely fresh `migrate deploy`) previously failed re-creating objects `0_baseline` already created. Both redundant migrations were deleted from git; their orphaned `_prisma_migrations` rows were cleared from the real database. `backend/prisma/migrations/` now contains, in order: `0_baseline`, `phase4_tenant_id_everywhere`, `user_tenant_memberships`, `equipment_details_json`, `schedule_slot_guides` — a clean, replayable, linear history.
...
**Fixed (Phase 6.11, 2026-08-10):**
- `data tanks.csv` — a loose file at the repo root containing real tank inventory (serial numbers, hydrostatic/visual test dates, storage notes), not test fixture data. A `docs/archive/` chat export shows it was a one-off bulk-import attempt for the Tanks/Cylinders feature that landed in the repo by accident rather than a deliberate commit. `git rm`'d from the working tree. Same caveat as the two items above applies — it remains in git history (introduced at commit `9157c30`) unless a separate history-rewrite pass is done; not attempted here since it's operational data (test dates, storage locations), not customer PII on the same order as the invoice PDF, and the user asked specifically for it to be removed from the repo, not for a history purge.
