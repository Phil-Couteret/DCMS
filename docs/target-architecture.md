...
- Composite uniqueness moves from global to per-tenant where appropriate: `(tenant_id, username)`, `(tenant_id, email)` on `users`; `(tenant_id, contact_email)` on `partners`; `(tenant_id, key)` on `settings`; `(tenant_id, email)` on `customers`. `partners.api_key` stays globally unique (it's a bearer credential, not a display field).
