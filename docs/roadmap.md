...
`npx tsc --noEmit` passed clean at the time this was written. **2026-07-17, later the same day:** found and fixed an unrelated, uncommitted `schema.prisma` change (`locations.type` had been turned into a fixed 3-value enum, contradicting the already-shipped, documented dynamic Location Types feature — see the "stale multi-tenant admin bootstrap work" commit). That fix requires a `prisma generate` to pick up; if you're seeing a `location_type` type error in `locations.service.ts`/`tenants.service.ts`, run `cd backend && npm run prisma:generate` first.

## Phase 3 — Test safety net (before touching the data model further)

**Status: items 1–2 written (2026-07-17); item 2 (e2e) not run/verified in this environment — see note.**
...
2. ✅ Wrote `backend/test/unauthenticated-routes.e2e-spec.ts` and `backend/test/tenant-isolation.e2e-spec.ts`.
   - `unauthenticated-routes.e2e-spec.ts` boots the real app (with `PrismaService` stubbed out, since a rejection happens in the guard before any DB call) and uses Nest's own `DiscoveryService`/`MetadataScanner` to enumerate every registered route, then asserts every route that isn't genuinely credential-free gets a 401 with no `Authorization` header. This distinguishes `@Public()` (skips only the global admin guard) from "no auth at all" — `partner/*`, `partner/bookings`, `partner/customers`, and `partner-invoices/my-invoices` are `@Public()` but carry their own `@UseGuards(PartnerAuthGuard | JwtPartnerGuard)`, discovered generically via Nest's `GUARDS_METADATA`, so they're correctly expected to still 401. Only `AppController` (root/health) and the 3 login handlers are treated as truly open. Because it discovers routes at runtime instead of a hand-maintained list, a new controller or endpoint that forgets a guard is caught automatically.
   - `tenant-isolation.e2e-spec.ts` seeds two real tenants + a customer each via Prisma, signs real JWTs via `AuthService`, and asserts tenant A's token can't list, read, update, or delete tenant B's customer (all correctly 404, since `CustomersService` filters by `tenant_id` before any write), plus that a header disagreeing with the JWT still gets the Phase 0 403. This needs a real, reachable Postgres and a Prisma client generated against the current schema — neither available in the sandbox this was written in (no network path to Prisma's engine binaries for this environment's architecture; see the bcrypt/Prisma-generate notes elsewhere in this doc and in `PRISMA_MIGRATION_BASELINE.md`). Not run here. Run locally with `cd backend && npm run test:e2e -- tenant-isolation` once `prisma:generate` is current.
   - `unauthenticated-routes.e2e-spec.ts` needs no real DB (confirmed by static/manual review of the Nest APIs it uses — `DiscoveryService.getControllers()`, `MetadataScanner.getAllMethodNames()`, `Reflector.get()`, `GUARDS_METADATA` — against the installed `@nestjs/core` type definitions), but this session's stale Prisma client (see Phase 2 note above) currently blocks `ts-jest` from compiling anything that imports `AppModule` at all, including this file. Run `npm run prisma:generate` first, then `npm run test:e2e -- unauthenticated-routes`.
3. Given item 1's unit coverage and item 2 being written (if not yet run), the intent of this gate is substantially met, but treat Phase 4 as still blocked until you've actually run both e2e suites locally and confirmed they're green.
